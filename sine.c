...
static unsigned int buffer_time = 6000;               /* ring buffer length in us */
                          ...
                          int count, double *_phase)
{
        ...
        unsigned int maxval = volume * (1 << (format_bits - 1)) - 1;
...
void *read_freq(void *arg) {
    double tfreq = 50, tvol = 1.0;
    while(1) {
        scanf("%lf %lf", &tfreq, &tvol);
        if(tfreq==0) continue;
        if(tfreq < 10) tfreq = 10;
        else if(tfreq > 5000) tfreq = 5000;
        if(tvol < 0.01) tvol = 0.01;
        else if(tvol > 1.0) tvol = 1.0;
        freq = tfreq;
        volume = tvol;
    }
}
int main(int argc, char *argv[])
{
        ...
        pthread_t tfreq;
        pthread_create(&tfreq, NULL, read_freq, NULL);
        err = transfer_methods[method].transfer_loop(handle, samples, areas);
        if (err < 0)
                printf("Transfer failed: %s\n", snd_strerror(err));
        pthread_join(tfreq, NULL);
        free(areas);
        free(samples);
